...
constexpr static std::string_view POST_ONLY = "POST"sv;
constexpr static std::string_view GET_AND_HEAD = "GET, HEAD"sv;
constexpr static std::string_view GET_ONLY = "GET"sv;
};  // namespace HTTPMethods
...
std::pair<bool, std::string_view> GetAuthToken(std::string_view auth) {
    if (auto pos = auth.find(BEARER_TOKEN_FIELD) != auth.npos) {
        return {false, auth.substr(pos + BEARER_TOKEN_FIELD.size() - 1)};
    }
    return {true, {}};
}
...
StringResponse RequestHandler::AddNewPlayer(const http_server::StringRequest& req) {
    ...
    if (req_content_type != http_server::ContentType::JSON) {
        return ReportServerError(model::error_code::INVALID_ARGUMENT, "Invalid content type"sv, req,
                                 http::status::bad_request, HTTPMethods::POST_ONLY,
    ...
    if (req.method() != http::verb::post) {
        return ReportServerError(model::error_code::INVALID_METHOD, "Only POST method is expected"sv, req,
                                 http::status::method_not_allowed, HTTPMethods::POST_ONLY,
                                 {http::field::cache_control, "no-cache"sv});
    }
    try {
        ...
        if (!err.empty() || !player) {
            return ReportServerError(model::error_code::MAP_NOT_FOUND, "Map not found"sv, req, http::status::not_found,
                                     HTTPMethods::POST_ONLY, {http::field::cache_control, "no-cache"sv});
        }
...
StringResponse RequestHandler::GetAllPlayers(const http_server::StringRequest& req) {
    if (req.method() != http::verb::get && req.method() != http::verb::head) {
        return ReportServerError(model::error_code::INVALID_METHOD, "Invalid method"sv, req,
                                 http::status::method_not_allowed, HTTPMethods::GET_AND_HEAD,
                                 {http::field::cache_control, "no-cache"sv});
    }
    const auto [err, token] = GetAuthToken(req[http::field::authorization]);
    if (!err) {
        // Получаем список игроков в виде response_body для игрока с токеном token
        try {
            auto [players, _] = game_.GetPlayersAndItems(token);
            if (!players) {
                auto response = ReportServerError(model::error_code::UNKNOWN_TOKEN, "Player token has not been found"sv,
                                                  req, http::status::unauthorized, HTTPMethods::GET_AND_HEAD,
                                                  {http::field::cache_control, "no-cache"sv});
                return response;
            }
            if (req.method() == http::verb::head) {
                auto response = http_server::MakeStringResponse(
                    http::status::ok, "", req, http_server::ContentType::JSON, HTTPMethods::GET_AND_HEAD,
                    {http::field::cache_control, "no-cache"sv});
...
StringResponse RequestHandler::GetGameState(const http_server::StringRequest& req) {
    ...
    if (!err) {
        // Получаем список игроков в виде response_body для игрока с токеном token
        try {
            auto [players, items] = game_.GetPlayersAndItems(token);
            if (!players) {
                auto response = ReportServerError(model::error_code::UNKNOWN_TOKEN, "Player token has not been found"sv,
                                                  req, http::status::unauthorized, HTTPMethods::GET_AND_HEAD,
                                                  {http::field::cache_control, "no-cache"sv});
            ...
            if (req.method() == http::verb::head) {
                ...
                auto body = boost::json::serialize(json::value_from(std::make_pair(players, items)));
                auto response = http_server::MakeStringResponse(
                    http::status::ok, body, req, http_server::ContentType::JSON, HTTPMethods::GET_AND_HEAD,
                    {http::field::cache_control, "no-cache"sv});
                return response;
        ...
        if (!err) {
            ...
            if (!game_.FindAndMovePlayersDog(token, json_loader::ParseDogMoveRequest(req.body()))) {
                return ReportServerError(model::error_code::UNKNOWN_TOKEN, "Player token has not been found"sv, req,
                                         http::status::unauthorized, HTTPMethods::POST_ONLY,
                                         {http::field::cache_control, "no-cache"sv});
            } else {
    ...
    try {
        ...
        if (err) {
            return ReportServerError(model::error_code::INVALID_ARGUMENT, "Failed to parse tick request JSON"sv, req,
                                     http::status::bad_request, HTTPMethods::GET_AND_HEAD,
                                     {http::field::cache_control, "no-cache"sv});
        } else {
...
StringResponse RequestHandler::GetMapByID(const http_server::StringRequest& req, const std::vector<std::string>& url) {
    ...
    if (it == maps.end()) {
        body =
            json::serialize(json::value_from(model::ResponseError{model::error_code::MAP_NOT_FOUND, "Map not found"}));
    ...
    if (verifyRequestPath(full_path_to_file, path_to_static_folder_)) {
        ...
        if (sys::error_code ec; file.open(full_path_to_file.c_str(), beast::file_mode::read, ec), ec) {
            auto body = json::serialize(
                json::value_from(model::ResponseError{model::error_code::FILE_NOT_FOUND, "Fail not found"}));
        ...
        // Make error response
        auto body = json::serialize(
            json::value_from(model::ResponseError{model::error_code::FILE_NOT_FOUND, "File Not Found"}));
        return http_server::MakeStringResponse(http::status::not_found, std::move(body), req,
                                               http_server::ContentType::TEXT_PLAIN);
    }
...
StringResponse RequestHandler::HandleApiRequest(const StringRequest& request) {
    ...
    } else if (target == url::CHANGE_TIME && !timer_) {
        return ChangeTime(request);
    } else if (target.starts_with(url::GET_INFO_FROM_DB)) {
        return GetInfoFromDB(request);
    }
    return ReportServerError(model::error_code::BAD_REQUEST, "Invalid endpoint"sv, request, http::status::bad_request,
                             {}, {http::field::cache_control, "no-cache"sv});
}

StringResponse RequestHandler::GetInfoFromDB(const http_server::StringRequest& req) {
    if (req.method() != http::verb::get) {
        return ReportServerError(model::error_code::INVALID_METHOD, "Invalid method"sv, req,
                                 http::status::method_not_allowed, HTTPMethods::GET_ONLY,
                                 {http::field::cache_control, "no-cache"sv});
    }
    try {
        auto [start, limit] = json_loader::GetParametersFromUrl(req.target());
        if (limit > 100) {
            return ReportServerError(model::error_code::INVALID_ARGUMENT, "Max limit is 100 records"sv, req,
                                     http::status::bad_request, HTTPMethods::GET_ONLY,
                                     {http::field::cache_control, "no-cache"sv});
        }
        auto players_data = game_.GetInfoFromDB(start, limit);
        auto body = json::serialize(json::value_from(players_data));
        return http_server::MakeStringResponse(http::status::ok, body, req, http_server::ContentType::JSON,
                                               HTTPMethods::POST_ONLY, {http::field::cache_control, "no-cache"sv});
    } catch (std::exception& err) {
        return ReportServerError(model::error_code::INVALID_ARGUMENT, "Failed to parse tick request JSON"sv, req,
