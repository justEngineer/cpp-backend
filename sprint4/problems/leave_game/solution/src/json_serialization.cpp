...
void tag_invoke(boost::json::value_from_tag, boost::json::value& jv, std::list<Player> const& players) {
    ...
    for (const auto& player : players) {
        ...
        object_name[json_get_players_response::LIST_PLAYERS_NAME] = player.info_.name_;
...
void tag_invoke(boost::json::value_from_tag, boost::json::value& jv,
                std::pair<const std::list<Player>*, const ItemsHash*> const& wrapper) {
...
void tag_invoke(boost::json::value_from_tag, boost::json::value& jv, std::vector<Player::PlayerInfo> const& info) {
    json::array records;
    for (const auto& player_info : info) {
        json::object object_record;
        object_record[json_record::NAME] = json::value_from(player_info.name_);
        object_record[json_record::SCORE] = json::value_from(player_info.score_);
        object_record[json_record::PLAY_TIME] =
            json::value_from(std::chrono::duration<double>(player_info.play_time_).count());
