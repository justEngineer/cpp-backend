...
constexpr std::string_view GET_INFO_FROM_DB = "/api/v1/game/records"sv;
...
class RequestHandler : public std::enable_shared_from_this<RequestHandler> {
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        ...
        try {
            if (req.target().starts_with(url::API)) {  // запрос API
                auto handle = [self = shared_from_this(), send = std::move(send), req = std::move(req), version,
                               keep_alive] {
                    try {
                        assert(self->api_strand_.running_in_this_thread());
                        return send(self->HandleApiRequest(req));
                    } catch (std::exception& ec) {
                        BOOST_LOG_TRIVIAL(warning) << "Exception while handling api request, error code: " << ec.what();
                        send(self->ReportServerError("Internal error"sv, "Exception while handling api request"sv,
                                                     std::move(req)));
                    }
                };
            ...
            BOOST_LOG_TRIVIAL(warning) << "Exception while handling request, error code: " << ec.what();
   ...
   private:
    ...
    StringResponse GetInfoFromDB(const http_server::StringRequest& req);
    model::Game& game_;
