...
class Dog {
   public:
    ...
    std::string_view GetDirectionAsString() const noexcept { return DirectionToString[direction_]; }
...
class Player {
   public:
    ...
    Player(uint64_t id, const std::string& name, const std::string& token, Dog dog)
        : id_(id), name_(name), token_(token), dog_(dog) {}
