...
void LogServerIsStarted(const std::string& address, unsigned int port, const std::string& message) {
    ...
    resp_object[model::json_log::LOGGER_MESSAGE] = message;
    resp_object[model::json_log::LOGGER_TIMESTAMP] = GetLogTime();

    json::object data_object;
    data_object[model::json_log::SERVER_ADDRESS] = address;
    data_object[model::json_log::SERVER_PORT] = port;

    resp_object[model::json_log::LOGGER_DATA] = data_object;
...
void LogServerIsShuttingDown(const std::string& message, int code, const std::string& exception_msg) {
    ...
    json::object data_object;
    data_object[model::json_log::RESPONSE_CODE] = code;
    if (!exception_msg.empty())
        data_object[model::json_error::EXCEPTION] = exception_msg;
...
void LogRequest(const std::string& ip_address, const std::string& uri, std::string_view http_method) {
    json::object resp_object;
    resp_object[model::json_log::LOGGER_MESSAGE] = std::string(logger::message::REQUEST_RECEIVED);
    resp_object[model::json_log::LOGGER_TIMESTAMP] = GetLogTime();

    json::object data_object;
    data_object[model::json_log::REQUEST_IP] = ip_address;
    data_object[model::json_log::REQUEST_URI] = uri;
    data_object[model::json_log::REQUEST_METHOD] = std::string(http_method);
