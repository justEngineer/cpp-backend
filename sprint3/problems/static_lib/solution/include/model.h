...
class Map {
   public:
    ...
    Map(Id id, std::string name, double dogSpeed, json::array loot_types) noexcept
        : id_(std::move(id)), name_(std::move(name)), dogSpeed_(dogSpeed), loot_types_(loot_types) {}
...
struct Item {
    ...
    Item(Id id, uint32_t type, Position position) noexcept : id_{std::move(id)}, type_{type}, position_{position} {}
