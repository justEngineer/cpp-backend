...
Building tag_invoke(json::value_to_tag<Building>, json::value const& jason_value) {
    ...
    return Building({value_to<Coord>(obj.at({json_config::map::building::POS_X})),
...
Office tag_invoke(json::value_to_tag<Office>, json::value const& jason_value) {
    ...
    return Office{Office::Id(value_to<std::string>(obj.at({json_config::map::office::ID}))),
...
Map tag_invoke(json::value_to_tag<Map>, json::value const& jason_value) {
    auto obj = jason_value.as_object();
    Map map{Map::Id(value_to<std::string>(obj.at({json_config::map::ID}))),
...
void tag_invoke(json::value_from_tag, json::value& jason_value, Office const& office) {
    ...
    jason_value = {{json_config::map::office::ID, id},
...
void tag_invoke(json::value_from_tag, json::value& jason_value, Building const& building) {
    ...
    jason_value = {{json_config::map::building::POS_X, bounds.position.x},
...
void tag_invoke(json::value_from_tag, json::value& jason_value, Road const& road) {
    ...
    if (start.y == end.y) {
        jason_value = {{json_config::map::road::START_X, start.x},
...
// void tag_invoke(boost::json::value_from_tag, boost::json::value& jv, model::Player const& player) {
//     json::object object_name;
//     object_name[json_add_player_request::NAME] = player.name_;
//     json::object object;
