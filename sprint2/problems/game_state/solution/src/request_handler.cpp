...
StringResponse RequestHandler::GetAllPlayers(const http_server::StringRequest& req) {
    ...
    auto token = GetAuthToken(req[http::field::authorization]);
    if (!token.empty()) {
...
StringResponse RequestHandler::HandleApiRequest(const StringRequest& request) {
    ...
    //return ReportServerError("badRequest"sv, "Bad request"sv, request);
    //    } else if (request.method() == http::verb::post) {
    // аутентификация нового игрока
    else if (target == url::PLAYER_AUTH) {
        return AddNewPlayer(request);
        //}
        //    } else if (request.method() == http::verb::post && target.starts_with(url::GET_ALL_PLAYERS)) {
        // else if (request.target() == url::GET_ALL_PLAYERS) {
        //     return GetAllPlayers(request);
    } else if (target == url::GET_STATE) {
        return GetGameState(request);
    } else {
        auto response = ReportServerError(model::error_code::INVALID_METHOD, "Invalid method"sv, request,
